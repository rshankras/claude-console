...
**Nothing appears after a first install on a machine that never ran the plugin before.** A sideloaded `.lplug4` install doesn't create the application entry the icon and layout hang off — only Logitech Marketplace installs get that step. **2.0.0 and later handle it automatically**: a few seconds after the install, the plugin writes its own registration and restarts the service (Options+ blinks once), and the icon plus the 9-key layout appear. On older versions, import the layout manually (profile menu → Import → the `.lp5` from the release).
