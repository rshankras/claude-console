# Vizhi for Codex 1.4.7 — preview notes
...
found three porting gaps — session discovery, the state bridge, the process scan's name
filter, and a hook exit-code edge; 1.4.7 fixes all four, unit-pinned. Hardware verification so far: profile registration,
...
2. Double-click `VizhiCodex_1.4.7.lplug4` → install via Options+.
