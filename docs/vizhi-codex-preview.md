# Vizhi for Codex 1.4.9 — preview notes
...
(kernel parent lookups instead of PowerShell spawns), and one-key-per-session de-duplication
(codex runs as a TUI process plus a child app server); 1.4.9 fixes all of it, unit-pinned. Hardware verification so far: profile registration,
...
2. Double-click `VizhiCodex_1.4.9.lplug4` → install via Options+.
