# Vizhi for Codex 1.4.8 — preview notes
...
found three porting gaps — session discovery, the state bridge, the process scan's name filter, a hook exit-code edge,
and one-key-per-session de-duplication (codex runs as a TUI process plus a child app server);
1.4.8 fixes all five, unit-pinned. Hardware verification so far: profile registration,
...
2. Double-click `VizhiCodex_1.4.8.lplug4` → install via Options+.
