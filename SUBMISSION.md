...
- [x] **The settings.json edit is reversible and disclosed** (#31). A **notice is posted in Options+** (`OnPluginStatusChanged`, Warning + link) on the load that writes the wiring, naming what was added, the backup and the undo command — the platform's nearest thing to the prompt QA asked for — and cleared on the next load; the install text says the plugin edits `~/.claude/settings.json`; the backup is rolling (rewritten before every change, never a stale first-load snapshot); `uninstall.sh --unwire` removes only the plugin's entries and restores a chained status line; the opt-out file is a two-way switch (create it later and the plugin unwires itself on next load). Verify on the clean-machine pass: wire, edit an unrelated setting, `--unwire`, confirm the unrelated edit survives and the backup holds the pre-unwire state.
