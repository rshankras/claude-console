...
- [x] **The settings.json edit is opt-in, reversible and disclosed** (#31). Nothing is written on install or load. **Enable Live Status** (group *Setup & Privacy*; on the layout download beside Cost) is the only path that edits `~/.claude/settings.json`, and its description states the change before the press; a **notice is posted in Options+** (`OnPluginStatusChanged`, Normal + link) the moment it writes. A live key pressed before setup flashes *Set up first*, posts a card naming the switch, and edits nothing. **Disable Live Status** (or `uninstall.sh --unwire`) removes only the plugin's entries, restores a chained status line, and leaves the Off marker; the backup is rolling (rewritten before every change, never a stale snapshot). The public SDK exposes no plugin-wide settings page (`PluginPreferenceType` is `{None, Account}`, and the account control badges every key while signed out), so this is the closest discoverable control it offers — say so to QA and ask them to confirm. Verify on the clean-machine pass per [docs/clean-install-test.md](docs/clean-install-test.md) step 5.
